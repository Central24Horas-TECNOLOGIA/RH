...
| --- | --- |
| `system.py` | Health/debug |
| `auth.py` | Login, sessão e logout |
| `history.py` | Histórico de provas e arquivos de resposta |
| `email_inbox.py` | Caixa de e-mail de currículos |
| `processes.py` | Processos, candidatos, banco de talentos, CVs e ações |
| `public_candidacy.py` | Candidatura pública por slug |
...
O backend padroniza respostas de erro como:
...
- exceções genéricas;
- erros `pyodbc.Error`;
- deadlock de banco, retornando indisponibilidade temporária.

## Autenticação

A autenticação é local, com usuário/senha configuráveis. Ao fazer login, a API retorna token de acesso. O frontend armazena a sessão e envia token nas chamadas protegidas.

Variáveis principais:

```env
RH_AUTH_USER=rh.local
RH_AUTH_PASSWORD=troque-esta-senha
RH_AUTH_TOKEN_SECRET=troque-este-segredo
RH_AUTH_TOKEN_TTL_MINUTES=480
```

## CORS

Em desenvolvimento, o backend permite origens locais como `localhost`, `127.0.0.1` e regex para ambiente interno. Em produção, é recomendável restringir para o endereço real do frontend.

## Observação sobre rotas duplicadas de e-mail

Existem endpoints de caixa de e-mail tanto em `email_inbox.py` quanto em `processes.py`. Isso indica compatibilidade histórica. O ideal é manter novas evoluções no router específico `email_inbox.py` e preservar as rotas antigas apenas para não quebrar frontend legado.
